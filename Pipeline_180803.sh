...
(for BARCODE in $BARCODE_ARR;do
    ...
    ${MERGE_RESCUE_DIR}${BARCODE}.fastq > ${OUTPUT_DIR}/${BARCODE}_prechemira.fastq \
    && ${PYTHON_PATH} ${SCR_SEQ_CONVERTER} ${OUTPUT_DIR}${BARCODE}_prechemira.fastq ${OUTPUT_DIR}${BARCODE}_prechemira.fna \
    && ${MAKEBLASTDB_PATH} -in ${OUTPUT_DIR}${BARCODE}_prechemira.fna -dbtype nucl -hash_index -parse_seqids \
    && ${CHIMERIC_PATH} -m usearch61 --suppress_usearch61_ref -i ${OUTPUT_DIR}${BARCODE}_prechemira.fna -o ${OUTPUT_DIR}${BARCODE}/ \
    && ${BLASTDBCMD_PATH} -db ${OUTPUT_DIR}${BARCODE}_prechemira.fna -entry_batch ${OUTPUT_DIR}${BARCODE}/non_chimeras.txt -out ${OUTPUT_DIR}${BARCODE}.fna"
done) > CAT.com
